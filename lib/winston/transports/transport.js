...
Transport.prototype.logException = function (msg, meta, callback) {
  ...
  if (meta && this.prettyPrint) {
    ...
    if (meta.process && meta.process.memoryUsage) {
      ...
      for (var key in mem) {
        mem[key] = addUnit(mem[key]); // Make memoryUsage numbers more readable, add units to it
      }
    }
  }
  if (meta && meta.process && this.prettyPrint) {
...
function addUnit(num) {
  ...
  return (pre === -1 ? num : num + prefixes[pre]) + 'B';
}
